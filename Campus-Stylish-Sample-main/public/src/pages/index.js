...
function Recommendations() {
	...
	return (
		<React.Fragment>
			<div className="recommendations_title">今夏最時尚</div>
			<div className="recommendations">
				{recommendations.map((recommendation) => (
					<a
						className="recommendation"
						key={recommendation.product_id}
						href={`./product.html?id=${recommendation.product_id}`}
					>
						<img src={`http://localhost:3000/assets/` + recommendation.product_id + `/main.jpg`} className="recommendation__image" />
						<div className="recommendation__title">{recommendation.detail[0].title}</div>
					</a>
				))}
...
function Products() {
	...
	return (
		<React.Fragment>
			<div className="products_title">商品總覽</div>
